...
let drawMapObj = function ({
  ...
  function setCanvasData () {
    baseConfig.data.json = data.filter(i => i.latitude && i.latitude);
  ...
  function setColorRange () {
    ...
    let colors = notEmpty(canvasCss.colors) ? canvasCss.colors.filter(c => c.colorList).map(c => c.colorList) : [];
  ...
  function getTooltipList () {
    let tooltipList = [];
    let name = [];
    let areas = isDefined(featureList.area) ? featureList.area.map(v => v.name) : [];
    areas.forEach(a => {
      tooltipList.push({
        type: 'ordinal',
        feature: a
    ...
    notEmpty(featureList.labels) && featureList.labels.forEach(l => {
...
let drawMap = function ({
  ...
  // 获取tooltipsList
  let { uniqueList: tooltipList, uniqueName: name } = drawMapFun.getTooltipList();
  baseConfig.tooltip.name = name;
  // 画图
  // let ins = mc.generate(baseConfig);

  // 获取style
  // let styles = drawMapFun.getStyles(ins);
  // return {
  //   ins,
  //   tooltipList,
  //   styles
