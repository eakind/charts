...
export default class Base {
  ...
  initYAxisHeight () {
    ...
    if (yAxisPart && yAxisPart.length) {
      ...
      let yAxisPartMap = setUniqueForKey(xAxis[0].key, yAxisPart[len].key, data);
      this.lastYPartList = [];
      ...
      // this.lastYPartList = [...new Set(yAxisPartList)];
      // 每个子画布中每个轴的高度
  ...
  initXAxisConfig () {
    let { xAxis, xAxisPart } = this.config;
    let data = this.data[0];
    // 获取X轴刻度数组
    this.xAxisList = [];
    if (xAxisPart && xAxisPart.length) {
      let xAxisPartMap = setUniqueForKey(xAxisPart[0].key, xAxis[0].key, data);
      for (let key in xAxisPartMap) {
        this.xAxisList.push(...xAxisPartMap[key]);
      }
    } else {
      this.xAxisList = [...new Set(getKeyDataList(data, xAxis[0].key))];
    };
  ...
  initCombinedYAxisConfig (yAxis) {
    ...
    let { leftAxisWidth, leftTitleWidth } = getMaxValueWidth(yAxis, this.data, yAxisPart, 'left');
    /* 左边坐标轴宽度 */
  ...
  drawCombinedYPart (yAxisPart, yAxisChild, index, len) {
    ...
    for (let i = 0; i < partLen; i++) {
      ...
      // createYAxisPartHorGrid(this.leftAxis, this.yAxisHeight * len, this.leftAxisWidth, this.topAxisHeight);
    }
  };
