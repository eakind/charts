...
export default {
  ...
  mounted () {
    ...
    this.setMobileLayoutList([]);
    this.mobileLayout = this.mobileLayoutList;
    this.setMobileList(this.layoutList);
    this.setMobileLayoutList(this.mobileLayout);
    this.reload();
  },
  watch: {
    layoutList: {
  ...
  computed: {
    ...mapState('dashboard', ['selectedTarget', 'currentTarget', 'mobileLayoutList', 'layoutList'])
  },
  methods: {
    ...
    setMobileList (list) {
      for (let i = 0; i < list.length; i++) {
        if (list[i].type && list[i].type !== 'filter') {
          ...
          obj.dir = 'row';
          obj.children = [];
          if (list[i].type === 'title' && !list[i].mobileHeight) {
            list[i].mobileHeight = 30;
          }
          if (list[i].type === 'canvas' && !list[i].mobileHeight) {
            list[i].mobileHeight = 360;
          }
          this.mobileLayout.push(obj);
        }
        this.setMobileList(list[i].children);
      }
    },
    reload () {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (this.mobileLayoutList) this.refreshAllCanvas(this.mobileLayoutList);
        });
      });
    },
    // 选中展板中的画布
