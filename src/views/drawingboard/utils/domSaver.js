...
function downloadImage ({ node, name = 'snapshot.png', size = { width: 1920, height: 1080 }, backgroundColor = 'transparent', errorMsg = '画布尺寸太大，无法保存为图片！将视图更改为“完整视图”后重试。' }) {
  return new Promise((resolve, reject) => {
    if (!(node instanceof SVGElement)) {
      _dom2canvas({ node, size, backgroundColor, scale: 1.5 }).then(canvas => {
...
function _dom2canvas ({ node, size = { width: 1920, height: 1080 }, backgroundColor = 'transparent', scale = 1.5 }) {
