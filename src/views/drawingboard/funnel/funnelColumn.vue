<template>
  <div class="column">
    <div
      class="left"
    >
      <span class="text">{{$t('message.dimensions')}}</span>
    ...
    <div class="right">
      ...
      >{{$t('message.drop_cat_features_max_one')}}</span>
...
export default {
  ...
  mounted () {
    ...
    this.bus.$on('drillFeature', (el, target) => {
      if (target) {
    ...
    this.bus.$on('drillRemove', (el, target) => {
  ...
  methods: {
    ...
    dropHandler (el) {
      ...
      this.getParam(dataType, featureType, featureName, 'column');
    },
    checkDuplicate (featureName) {
      const list = this.canvasFeatures.x || [];
